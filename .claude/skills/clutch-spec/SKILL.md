...
> **Why `disable-model-invocation: true` is set:** this skill is the human's
> half of the approval gate. It must never fire on its own — not from a
> `/loop` iteration, not from a `/goal` turn, not from Claude deciding on its
> own that a spec would help. Only a human types `/clutch-spec`.
...
- Size the issue to one day of agent work or less. Bigger work becomes a
  ...
  code from the ones before it. (Explicit Linear blocked-by chaining lands
  in Clutch-loop v1.1 — for v1, just order the issues and say so in the
  Problem section of each.)
