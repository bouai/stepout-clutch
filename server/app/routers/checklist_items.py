...
def list_checklist_items(
    ...
    query = db.query(models.ChecklistItem).filter(
...
def _owned_inventory(
    db: Session, inventory_item_id: int, user: models.User
...
def create_checklist_item(
    ...
    if payload.inventory_item_id is not None:
        if _owned_inventory(db, payload.inventory_item_id, user) is None:
            raise HTTPException(status_code=404, detail="Inventory item not found")

    item = models.ChecklistItem(**payload.model_dump(), user_id=user.id)
    db.add(item)
    db.commit()
    db.refresh(item)
    return item
...
def update_checklist_item(
    ...
    if updates.get("inventory_item_id") is not None:
        if _owned_inventory(db, updates["inventory_item_id"], user) is None:
            raise HTTPException(status_code=404, detail="Inventory item not found")
    ...
    if "is_checked" in updates and item.inventory_item_id is not None:
        linked_item = _owned_inventory(db, item.inventory_item_id, user)
...
def _owned(db: Session, item_id: int, user: models.User) -> models.ChecklistItem | None:
    return (
